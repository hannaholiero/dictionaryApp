...
describe("Sidebar", () => {
  // Test för att kontrollera att sidebaren krymper vid liten skärm
  it("should shrink the sidebar on small screens", async () => {
    render(<App />);

    // Simulera liten skärm genom att ändra fönstrets storlek
    window.innerWidth = 500;
    window.dispatchEvent(new Event("resize"));

    // Öppna sidebaren via knappen
    const openButton = screen.getByRole("button", { name: /Open Sidebar/i });
    await userEvent.click(openButton);

    // Kontrollera att sidebaren har klassen som indikerar att den är öppen på liten skärm
    const sidebar = screen.getByTestId("sidebar");
    expect(sidebar).toHaveClass("fixed inset-y-0 right-0 z-20 w-64 p-4");

    // Stäng sidebaren och kontrollera att den döljs
